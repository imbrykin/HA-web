...
1. Zabbix доступен по **http://<zabbix_public_IP>**
2. Elastic доступен по **http://<elastic_public_ip>>:5601**. В Discover доступны nginx логи с трех [хостов](http://screenshot.alarislabs.com/ib2024/image_20240915211720_a5bb70ce.png)
3. Веб-сайт доступен по **http://<L7_public_IP>>**. Там же можно проверить работу балансировщика, обновляя страницу с помощью `F5` или `CTRL+F5` (для актуализации кеша). Сайт возвращает приватный IP хоста, на который пришел запрос.
